...
@router.callback_query(F.data == "view_user_activity")
async def view_user_activity(query: CallbackQuery):
    if query.from_user.id != ADMIN_ID: return
    
    stats = await db_call(db.get_user_activity_stats, 25)
    if not stats:
        await query.answer("ℹ️ Нет данных об активности", show_alert=True)
        return
        
    text_lines = ["📊 <b>Активность учеников (последние 25):</b>\n"]
    
    from datetime import datetime
    for user_id, username, first_name, last_active_at in stats:
        name = first_name or "Неизвестный"
        uname = f" (@{username})" if username else ""
        
        # Форматируем дату (может быть строкой из sqlite)
        date_str = last_active_at
        if isinstance(last_active_at, str) and " " in last_active_at:
            try:
                date_str = datetime.strptime(last_active_at, "%Y-%m-%d %H:%M:%S").strftime("%d.%m.%Y %H:%M")
            except Exception:
                pass
                
        text_lines.append(f"👤 {name}{uname}\n🕒 Последний вход: {date_str}\n―――――――――――――")
        
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="◀️ Выход", callback_data="admin_panel")]
    ])
