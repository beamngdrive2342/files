...
async def send_broadcast_to_users(bot: Bot, user_ids: list[int], text: str, reply_markup=None) -> dict[str, Any]:
    unique_user_ids = [user_id for user_id in dict.fromkeys(user_ids) if isinstance(user_id, int)]
    ...
    await asyncio.gather(*(_send(user_id) for user_id in unique_user_ids), return_exceptions=True)
    return {
...
async def send_notifications_to_users(bot: Bot, date: str, subject: str):
    ...
    await send_broadcast_to_users(bot=bot, user_ids=users, text=notification_text, reply_markup=keyboard)
